...
> 本文说明 fake-game-mod（**json 类型蓝本**）的「数据层」是什么，帮助 repo 作者理解 json vs csharp-dll 两种类型的数据层差异。给开发者看；agent 直接 grep `docs/` 即可。
...
一句话：**json 类型的数据层 = `docs/` 里手写的字段定义；没有 `docs/api/` + `docs/data/` 那种「从游戏提取的生成快照」，也就没有 refresh.py 这套刷新工具。**
